...
public class UltraBasicGenericMapper implements GenericMapper {
	...
	@Override
	public <S, D> D map(S source, Class<S> sourceClass, Class<D> targetClass) {
	...
	public /*static*/ <S,D> D map(S source , Class<D> targetClass) {
		return map(source,(Class<S>)source.getClass(),targetClass);
	}

	@Override
	public <S, D> List<D> map(List<S> sourceList, Class<S> sourceClass, Class<D> targetClass) {
		if(sourceList==null || targetClass==null) return null;
		return  sourceList.stream()
				.map((source)->map(source,sourceClass,targetClass))
