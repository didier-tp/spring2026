...
@SpringBootTest
@Slf4j
public class TestDaoCompteJpaDirect {

    @Autowired
    private DaoCompteJpaDirect daoCompte; //chose prise en charge par spring à tester

    @Test
    public void testAjoutRelecture(){
        Compte c1 = new Compte(null,"compte_1",200.0);
        this.daoCompte.save(c1); //déclenche INSERT INTO et auto_increment
        log.debug("c1 sauvegardé:" + c1.toString());
        Compte c1Relu = this.daoCompte.findById(c1.getNumero());
        log.debug("c1Relu="+c1Relu);
        assertEquals("compte_1",c1Relu.getLabel());
        assertEquals(200.0 , c1Relu.getSolde(), 0.000001);
